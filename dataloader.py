...
class Dataset(data.Dataset):
    def __init__(self, src_seq, trg_seq, index_seq ,src_word2id, trg_word2id,max_len, trg_plain):
    ...
    def preprocess_index(self, sequence, src_seq):
        """Converts words to ids."""
...
def get_seq(pairs,lang,batch_size,type,max_len):   
    ...
    for pair in pairs:
        ...
        if(type):
            lang.index_story(pair[0])
            lang.index_story(pair[1])
    
    dataset = Dataset(x_seq, y_seq,ptr_seq,lang.word2index, lang.word2index,max_len, trg_plain)
